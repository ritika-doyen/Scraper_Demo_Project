...
def auto_scroll(page, scroll_container_selector, scroll_count=6, delay=1.0, limit=None):
...
def extract_data(page, limit=None):
    ...
    for i, card in enumerate(cards):
        try:
            ...
            # Expand the card for more details
            card.click()
            time.sleep(2)

            sidebar = page.locator("div[role='main']")
            address = sidebar.locator("button[data-item-id='address']").first
            rating = sidebar.locator("span[aria-label*='stars']").first
...
def run_scraper(query, output_file=None, limit=None):
    ...
    with sync_playwright() as p:
        ...
        auto_scroll(page, "div.m6QErb.DxyBCb.kA9KIf.dS8AEf.XiKgde", scroll_count=6, delay=1.0, limit=limit)
